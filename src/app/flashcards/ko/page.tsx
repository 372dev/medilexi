      ...
      <div className="c-filter-bar">
        <div className="c-search-row">
          ...
          <div className="c-filter-row" style={{ marginBottom: 0 }}>
            <button
              className={`c-pill ${isKoEn ? 'c-pill--active' : ''}`}
              onClick={() => { setDirection('ko-en'); setStarted(false) }}
            >KO → EN</button>
            <button
              className={`c-pill ${!isKoEn ? 'c-pill--active' : ''}`}
              onClick={() => { setDirection('en-ko'); setStarted(false) }}
            >EN → KO</button>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          >🔀 Shuffle</button>
          <Link href="/glossary/ko" className="c-btn-pixel" style={{ fontSize: '0.46rem', padding: '0.4rem 0.8rem' }}>
            ← Glossary
          </Link>
        </div>
      </div>
                ...
                <div style={{
                  ...
                  {isKoEn ? (
                    <>
                      ...
                      {card.ko_l && (
                        <div className="ko-l" style={{ fontSize: '1.2rem' }}>{card.ko_l}</div>
                      )}
                    </>
                  ) : (
                    <>
                      <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--color-text)', textAlign: 'center', lineHeight: 1.3 }}>
                        {card.en_h}
                      </div>
                ...
                <div style={{
                  ...
                  {isKoEn ? (
                    <>
                      ...
                      {card.en_l && (
                        <div style={{ fontSize: '0.95rem', color: 'var(--color-text-dim)' }}>{card.en_l}</div>
                      )}
                      <p style={{ fontSize: '0.9rem', color: 'var(--color-text-dim)', lineHeight: 1.7 }}>{card.d}</p>
                    </>
                  ) : (
                    <>
                      <div className="ko-h" style={{ fontSize: '1.6rem' }}>{card.ko_h}</div>
                      {card.ko_l && <div className="ko-l">{card.ko_l}</div>}
                      <p style={{ fontSize: '0.9rem', color: 'var(--color-text-dim)', lineHeight: 1.7 }}>{card.d_ko}</p>
                    </>
                  )}
