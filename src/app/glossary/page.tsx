...
const partsMap = Object.fromEntries(
  (partsData as WordPart[]).map(p => [p.wp, p])
)

// Strip wp key to its literal form for matching
function toLiteral(wp: string): string {
  return wp
    .replace(/^-/, '').replace(/-$/, '')  // strip leading/trailing dashes
    .replace(/\/[oiea]$/, '')              // strip combining vowel
...
function getSegments(en_h: string, parts?: VocabEntry['parts']): Segment[] {
  ...
  // Build list of all parts with their literal and type
  const allParts: { literal: string; wp: string; type: 'p' | 'r' | 's'; meaning: string }[] = []
  const typeMap: Record<string, 'p' | 'r' | 's'> = {
    p: 'p', r: 'r', s: 's'
  }
  ...
  for (const [ptype, wpList] of Object.entries(parts)) {
    for (const wp of wpList as string[]) {
      ...
      allParts.push({ literal, wp, type: typeMap[ptype], meaning })
    }
  }

  if (allParts.length === 0) return [{ text: en_h }]

  // Find all match positions in en_h (case-insensitive)
  ...
  if (matches.length === 0) return [{ text: en_h }]
  ...
  for (const m of matches) {
    if (m.start > cursor) {
      segments.push({ text: en_h.slice(cursor, m.start) })
    }
    segments.push({
      text: en_h.slice(m.start, m.end),
      wp: m.wp,
      type: m.type,
      meaning: m.meaning
    })
  ...
  if (cursor < en_h.length) {
    segments.push({ text: en_h.slice(cursor) })
  }
...
// ── Card component ─────────────────────────────────────────────────
function GlossaryCard({ v }: { v: VocabEntry }) {
        ...
        {hovered && v.parts
          ? segments.map((seg, i) =>
              seg.wp ? (
                <span
                  key={i}
                  className={`${styles.partHighlight} ${PART_CLASS[seg.type!]}`}
                  data-tooltip={`${seg.wp} · ${seg.meaning}`}
                >
                  {seg.text}
                </span>
              ) : (
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.filterRow}>
        ...
        {ALL_LEVELS.map(lvl => (
          <button key={lvl} className={`${styles.pill} ${levelFilter === lvl ? styles.pillActive : ''}`} onClick={() => setLevel(levelFilter === lvl ? null : lvl)}>
            {LEVEL_STARS[lvl]}
          </button>
        ))}
      </div>
      ...
      <div className={styles.filterRow}>
        ...
        {ALL_FIELDS.map(f => (
          <button key={f} className={`${styles.pill} ${fieldFilter === f ? styles.pillActive : ''}`} onClick={() => setField(fieldFilter === f ? null : f)}>
            {f}
          </button>
        ))}
      </div>
